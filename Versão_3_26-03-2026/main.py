...
# def register_link():
#     return redirect(url_for('registerAdd'))
...
def loginRegister():
    ...
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('back/users.db')
        cursor = conn.cursor()
        ...
        password_db = user[5]
        tentativa_em_bytes = password_form.encode('utf-8')
        hash_salvo_em_bytes = password_db.encode('utf-8')
        if bcrypt.checkpw(tentativa_em_bytes, hash_salvo_em_bytes) and user[6]:
            session['logged_in'] = True
            session['username'] = user[2]
            session['admin'] = user[6]
            print(user[6])
            return redirect(url_for('registerAdd'))
        else:
    ...
    if 'logged_in' in session:
        if session['admin'] == 'on':
            if request.method == 'POST':
                ...
                email_form = request.form.get('email')
                tel_from = request.form.get('telefone')
                instituicao_form = request.form.get('instituicao')
                password_form = request.form.get('password')
                confirm_password_form = request.form.get('confirm_pass')
                adm_form = request.form.get('superusuario')
                print(adm_form)

                if password_form != confirm_password_form:
                    error = 'As senhas não são equivalentes entre os campos'
                    return render_template('register.html', error=error)

                # 2. Codificar a senha para bytes (o bcrypt exige)
                senha_em_bytes = password_form.encode('utf-8')

                # 3. Gerar o "salt" e criar o hash
                # O bcrypt.gensalt() gera um salt aleatório e seguro
                hash_da_senha = bcrypt.hashpw(senha_em_bytes, bcrypt.gensalt())

                # 4. O hash é o que você salva no banco de dados
                # Precisamos decodificá-lo de bytes para string para salvar em um campo de texto
                hash_para_salvar_no_db = hash_da_senha.decode('utf-8')
                
                try:
                    conn = sqlite3.connect('back/users.db')
                    cursor = conn.cursor()
                    if adm_form:
                        cursor.execute("INSERT INTO users (nome, email, telefone, instituicao, senha, superusuario) VALUES (?, ?, ?, ?, ?, ?)", 
                                        (nome_form, email_form, tel_from, instituicao_form, hash_para_salvar_no_db, adm_form))
                    else:
                        cursor.execute("INSERT INTO users (nome, email, telefone, instituicao, senha) VALUES (?, ?, ?, ?, ?)", 
                                        (nome_form, email_form, tel_from, instituicao_form, hash_para_salvar_no_db))
                    conn.commit()
                    conn.close()
                    return redirect(url_for('registerAdd'))
                except sqlite3.IntegrityError:
                    # Esta exceção ocorre se o login (que é UNIQUE) já existir
...
def login():
    ...
    if request.method == 'POST':
        ...
        tentativa_em_bytes = password_form.encode('utf-8')
        hash_salvo_em_bytes = password_db.encode('utf-8')
        if bcrypt.checkpw(tentativa_em_bytes, hash_salvo_em_bytes):
            session['logged_in'] = True
            session['username'] = user[2]
            session['admin'] = user[6]
            print(user[6])
...
def search():
    ...
    try:
        ...
        timeout_seconds = 20 * 60  # espera até 15 minutos
...
if __name__ == '__main__':
   # debug=True: Ativa o modo de depuração. O servidor reinicia com alterações
   # e, MAIS IMPORTANTE, mostra erros detalhados no navegador.
   app.run()
