...
llm_answer = [] # Deixado vazio conforme sua solicitação.

rouge = Rouge()

# Variáveis para armazenar o melhor resultado global ROUGE-L
best_f1_rouge_l = -1.0
best_recall_rouge_l = -1.0
best_precision_rouge_l = -1.0

best_llm_answers_set = None
current_execution_number = 0 # Para rastrear qual execução gerou o melhor resultado

# --- ARQUIVOS GERADOS ---
BEST_RESULTS_FILE = "best_rouge_l_results.json" # Para o melhor resultado individual
ALL_EXECUTIONS_HISTORY_FILE = "rouge_l_executions_history.json" # Para o histórico de todas as execuções
AVERAGE_RESULTS_FILE = "rouge_l_average_results.json" # Para a média final das 30 execuções
...
def load_best_results():
    """Carrega o melhor resultado ROUGE-L salvo em um arquivo."""
    try:
        with open(BEST_RESULTS_FILE, 'r') as f:
            data = json.load(f)
            return data.get('best_f1_rouge_l', -1.0), \
                   data.get('best_recall_rouge_l', -1.0), \
                   data.get('best_precision_rouge_l', -1.0), \
...
def save_best_results(f1, recall, precision, llm_answers, execution_num_of_best):
    """Salva o melhor resultado ROUGE-L em um arquivo."""
    data = {
        'best_f1_rouge_l': f1,
        'best_recall_rouge_l': recall,
        'best_precision_rouge_l': precision,
...
best_f1_rouge_l, best_recall_rouge_l, best_precision_rouge_l, best_llm_answers_set, _ = load_best_results()
...
for indice,vetor in enumerate(all_resps):
    ...
    if not llm_answer: # Verifica se o vetor llm_answer está vazio
        ...
        rouge_l_recall_sum = 0
        rouge_l_precision_sum = 0
        rouge_l_f1_sum = 0
        ...
        for i in range(num_comparisons):
            ...
            current_r = scores[0]['rouge-l']['r']
            current_p = scores[0]['rouge-l']['p']
            current_f = scores[0]['rouge-l']['f']
            ...
            current_individual_scores.append({
                ...
                'rouge_l_recall': current_r,
                'rouge_l_precision': current_p,
                'rouge_l_f1': current_f
            })

            # Acumula scores para ROUGE-L
            rouge_l_recall_sum += scores[0]['rouge-l']['r']
            rouge_l_precision_sum += scores[0]['rouge-l']['p']
            rouge_l_f1_sum += scores[0]['rouge-l']['f']

        # Calcular as médias para ROUGE-L da execução atual
        current_avg_rouge_l_recall = rouge_l_recall_sum / num_comparisons
        current_avg_rouge_l_precision = rouge_l_precision_sum / num_comparisons
        current_avg_rouge_l_f1 = rouge_l_f1_sum / num_comparisons


        print("\n--- Resultados da Execução Atual ---")
        print(f"ROUGE-L:")
        print(f"  Recall Médio: {current_avg_rouge_l_recall:.4f}")
        print(f"  Precisão Média: {current_avg_rouge_l_precision:.4f}")
        print(f"  F1-Score Médio: {current_avg_rouge_l_f1:.4f}")
        print("-" * 30)
        ...
        execution_history_data['executions'].append({
            ...
            'model': model,
            'rouge_l_recall': current_avg_rouge_l_recall,
            'rouge_l_precision': current_avg_rouge_l_precision,
            'rouge_l_f1': current_avg_rouge_l_f1,
        ...
        # Verifica se a execução atual é a melhor até agora para ROUGE-L
        ...
        if current_avg_rouge_l_f1 > best_f1_rouge_l:
            best_f1_rouge_l = current_avg_rouge_l_f1
            best_recall_rouge_l = current_avg_rouge_l_recall
            best_precision_rouge_l = current_avg_rouge_l_precision
            ...
            save_best_results(best_f1_rouge_l, best_recall_rouge_l, best_precision_rouge_l, best_llm_answers_set, f"Exec {current_execution_number} - Modelo {model}")
            ...
            prev_f1_l, prev_recall_l, prev_precision_l, prev_answers, prev_exec_num = load_best_results()
            save_best_results(prev_f1_l, prev_recall_l, prev_precision_l, prev_answers, prev_exec_num)
            print(f"\nNão foi o melhor resultado individual. O melhor ROUGE-L F1 continua sendo {best_f1_rouge_l:.4f} (da Execução {load_best_results()[4]})")


        print("\n--- Melhor Resultado ROUGE-L até agora ---")
        print(f"Melhor F1-Score (ROUGE-L): {best_f1_rouge_l:.4f} (da Execução {load_best_results()[4]})")
        print(f"  Recall Associado: {best_recall_rouge_l:.4f}")
        print(f"  Precisão Associada: {best_precision_rouge_l:.4f}")
        ...
        if current_execution_number == NUM_TOTAL_EXECUTIONS:
            ...
            total_recall = sum(exec['rouge_l_recall'] for exec in execution_history_data['executions'])
            total_precision = sum(exec['rouge_l_precision'] for exec in execution_history_data['executions'])
            total_f1 = sum(exec['rouge_l_f1'] for exec in execution_history_data['executions'])
            ...
            final_average_results = {
                ...
                'average_rouge_l_recall': avg_recall_30_executions,
                'average_rouge_l_precision': avg_precision_30_executions,
                'average_rouge_l_f1': avg_f1_30_executions
            }

            with open(AVERAGE_RESULTS_FILE, 'w') as f:
                json.dump(final_average_results, f, indent=4)

            print(f"\nMédias das {NUM_TOTAL_EXECUTIONS} execuções (ROUGE-L) salvas em '{AVERAGE_RESULTS_FILE}':")
