...
initialize_model(3,30)
dir_dest = "testes_3" 
os.makedirs(dir_dest, exist_ok=True)
thread_id=str(uuid.uuid4())
for k, i in enumerate(perguntas):
    resp = search(i,thread_id, 2)
    if k==3:
        arq_name = f"p5_seq_llama_faiss_v3_exec_{sys.argv[1]}.txt"
    elif k>=4 and k<9:
        arq_name = f"p{k+4}_seq_llama_faiss_v3_exec_{sys.argv[1]}.txt"
    elif k==9:
        arq_name = f"p14_seq_llama_faiss_v3_exec_{sys.argv[1]}.txt"
    else:
        arq_name = f"p{k+1}_seq_llama_faiss_v3_exec_{sys.argv[1]}.txt"
